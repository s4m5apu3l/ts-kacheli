...
?>
<?php
/* Template Name: foodcort detail page */
?>

<?php
// Получаем данные о текущей новости
$pods = pods('foodcort', get_queried_object_id());
...
if ($pods->exists()) {
  ...
  $date = $pods->field('date-time');
  $gallery = $pods->field('gallery');

  $contentNews = wpautop($content);
    ...
    <section class="max-[560px]:pt-[20px] pb-[200px] pt-[76px] border border-[#2D2D2D]">
      <div class="l-wrapper">
        ...
        <div class="max-[520px]:pb-[20px]  max-[520px]:px-[25px] max-[900px]:px-[50px] px-[100px] pb-[60px] mt-[-11%] bg-[#1C1C1C]">
          <div style="word-break:break-word" class="max-[560px]:p-[20px] max-[560px]:px-0 max-[768px]:text-[20px] max-[768px]:leading-[23px] relative z-10 max-w-[720px] mx-auto text-center p-[60px] bg-[#151515] text-[40px] text-[#F3F2EA] leading-[46px] font-bold">
            <?php echo $title ?>
          </div>
          <div class=" max-[425px]:mt-[22px] mt-[60px]">
            <div class="max-[768px]:text-base max-[768px]:leading-[24px] text-[24px] text-[#F3F2EA]  leading-[36px]">
              <?php echo $contentNews ?>
            </div>
          </div>
        </div>
